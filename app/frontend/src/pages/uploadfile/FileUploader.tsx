...
const FileUploader = () => {
    ...
    const handleUpload = async () => {
        if (!selectedFiles) {
            console.log("Nici un PDF selectat");
            return;
        }
        const formData = new FormData();
    ...
    if (uploadStatus.length > 0) {
        ...
        if (successCount === uploadStatus.length) {
            statusMessage = "Incarcat cu succes!";
        } else {
            statusMessage = `${successCount} of ${uploadStatus.length} fisiere urcate cu succes\n ${uploadStatus
            ...
            <div className={styles.uploadTopSection}>
                ...
                <div className={styles.fileInputContainer}>
                    <label htmlFor="file-upload" className={styles.uploadButton}>
                        {selectedFiles ? `Upload ${selectedFiles.length} files` : "Alegeti PDF de incarcat"}
                        <input id="file-upload" type="file" onChange={handleFileSelection} multiple className={styles.uploadInput} />
                    </label>
                </div>
            </div>
            <PrimaryButton className={styles.uploadButton} onClick={handleUpload} text="Incarca" />
